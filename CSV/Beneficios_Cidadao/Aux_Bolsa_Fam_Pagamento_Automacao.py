...
bucket_name = "aux-bolsa-fam-pagamento"
...
chrome_options.add_argument("--unsafely-treat-insecure-origin-as-secure=https://portaldatransparencia.gov.br/download-de-dados/bolsa-familia-pagamentos")
...
try:
    ...
    driver.get('https://portaldatransparencia.gov.br/download-de-dados/bolsa-familia-pagamentos')
    ...
    for option_ano in options_ano:
        ...
        for option_mes in options_mes:
            ...
            while download_in_progress:
                download_in_progress = not any(fname.endswith("_BolsaFamilia_Pagamentos.zip") for fname in os.listdir(download_path))
                logging.info("Aguardando a conclusão do download...")      
                time.sleep(5)

            # Processar arquivos baixados (ZIP)
            downloaded_files = [f for f in os.listdir(download_path) if f.endswith("_BolsaFamilia_Pagamentos.zip")]
